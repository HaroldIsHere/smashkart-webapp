...
<body>
    ...
    <div class="controls">
        <button onclick="prevImage()" class="prevBtn">
            < </button>
    ...
    <div class="onsale">
        ...
        <div class="slider-container">
            @foreach($bags as $bag)
                <a href="{{ route('products.bagView', ['name' => $bag->Name]) }}"
                    style="text-decoration:none; color:inherit;">
                    <div class="bag-card">
                        <img src="{{ asset('img/Products/Bag/' . $bag->Name . '.jpg') }}" alt="{{ $bag->Name }}"
                            style="width:100%; height:150px; object-fit:cover; border-radius:8px;">
                        <h3>{{ $bag->Name }}</h3>
                        <p><strong>Price:</strong> ₱{{ number_format($bag->SRP, 2) }}</p>
                        @if($bag->RC)
                            <span class="badge" style="cursor:pointer;"
                                onclick="showBadgeDesc(event, 'This bag has a Racket Compartment.')">
                                <img src="/img/Icons/RC.png" alt="RC" style="width:20px; height:20px; vertical-align:middle;">
                            </span>
                        @endif
                        @if($bag->SC)
                            <span class="badge" style="cursor:pointer;"
                                onclick="showBadgeDesc(event, 'This bag has a Shoe Compartment.')">
                                <img src="/img/Icons/SC.png" alt="SC" style="width:20px; height:20px; vertical-align:middle;">
                            </span>
                        @endif
                        <span id="badge-desc-popup"
                            style="display:none; position:absolute; background:#222; color:#fff; padding:8px 12px; border-radius:6px; font-size:14px; z-index:999;"></span>
                        <script>
                        ...
                        <a href="javascript:void(0);" onclick="addToCart('bag', {{ $bag->BagID }})" class="add-to-cart-btn">
                            <img src="/img/Icons/AddToCart.png" alt="Add to Cart"
    ...
    <div class="shoe-container">
        ...
        <div class="slider-container">
            @foreach($shoes as $shoe)
                <a href="{{ route('products.shoeView', ['name' => $shoe->Name]) }}"
                    style="text-decoration:none; color:inherit;">
                    <div class="bag-card">
                        <img src="{{ asset('img/Products/Shoes/' . $shoe->Name . '.jpg') }}" alt="{{ $shoe->Name }}"
                            style="width:100%; height:150px; object-fit:cover; border-radius:8px;">
                        <h3>{{ $shoe->Name }}</h3>
                        <p><strong>Price:</strong> ₱{{ number_format($shoe->SRP, 2) }}</p>
                        <a href="javascript:void(0);" onclick="addToCart('shoe', {{ $shoe->ShoeID }})"
    ...
    <div class="badminton-container">
        ...
        <div class="slider-container">
            @foreach($bads as $bad)
                <a href="{{ route('products.badView', ['name' => $bad->Name]) }}"
                    style="text-decoration:none; color:inherit;">
                    <div class="bag-card">
                        <img src="{{ asset('img/Products/Rackets/' . $bad->Name . '.jpg') }}" alt="{{ $bad->Name }}"
                            style="width:100%; height:150px; object-fit:cover; border-radius:8px;">
                        <h3>{{ $bad->Name }}</h3>
    ...
    <div class="apparel-container">
        ...
        <div class="slider-container">
            @foreach($apparels as $apparel)
                <a href="{{ route('products.appView', ['name' => $apparel->Name]) }}"
                    style="text-decoration:none; color:inherit;">
                    <div class="bag-card">
                        <img src="{{ asset('img/Products/Apparel/' . $apparel->Name . '.jpg') }}" alt="{{ $apparel->Name }}"
                            style="width:100%; height:150px; object-fit:cover; border-radius:8px;">
                        <h3>{{ $apparel->Name }}</h3>
                        <p><strong>Price:</strong> ₱{{ number_format($apparel->SRP, 2) }}</p>
                        <a href="javascript:void(0);" onclick="addToCart('apparel', {{ $apparel->AppID }})"
                            class="add-to-cart-btn">
                            <img src="/img/Icons/AddToCart.png" alt="Add to Cart"
                                style="width:20px; vertical-align:middle;">
                        </a>
